...
from db_optimization import (
    ...
    get_collection_stats, MONGO_POOL_CONFIG, create_critical_indexes_for_scale
)
...
# Import Cloudinary for image/video uploads
from cloudinary_service import (
    is_cloudinary_enabled, generate_upload_signature, 
    upload_image, upload_video, delete_asset, get_optimized_url
)
...
async def health_check():
    return {"status": "healthy", "app": "Nati Fenua", "version": "1.2.0", "optimized": True, "max_users": 5000}


@app.get("/api/status")
async def app_status():
    """Detailed status for monitoring - optimized for 5000 users"""
    try:
        # Quick DB check
        db_ok = False
        try:
        ...
        return {
            ...
            "version": "1.2.0",
            "optimized_for": "5000 concurrent users",
            "features": {
                "gzip_compression": True,
                "cors_middleware": True,
                "rate_limiting": True,
                "cloudinary_optimization": True,
...
async def startup_event():
    ...
    # 4. Create critical indexes for 5000+ users scale
    try:
        scale_indexes = await create_critical_indexes_for_scale(db)
        logger.info(f"✅ Scale indexes created: {scale_indexes['count']} indexes")
    except Exception as e:
...
@limiter.limit("10/minute")  # Rate limit: 10 login attempts per minute per IP (security)
...
async def google_callback(request: Request, response: Response, code: str = None, error: str = None):
    ...
    try:
        ...
        # Redirect to frontend with session token in URL fragment
        # Use fragment (#) instead of query (?) for better iOS/Safari compatibility
...
async def get_posts(
    request: Request, 
    limit: int = 20, 
    cursor: str = None,  # Cursor pagination: "created_at:post_id"
    skip: int = 0  # Deprecated, kept for backwards compatibility
):
    """Get posts with smart feed algorithm - cursor pagination for 5000+ users"""
    import random
    
    # Enforce limit bounds (max 30 for feed)
        ...
        try:
            ...
            cursor_filter = {
                "$or": [
                    ...
                    {"created_at": cursor_date, "post_id": {"$lt": cursor_id}}
                ]
            }
        except:
    ...
    # Optimized pipeline with minimal projection
    user_posts_pipeline = [
        {"$match": user_match},
        ...
        {"$match": {"$or": [{"user_data.is_bot": {"$ne": True}}, {"user_data": {"$exists": False}}]}},
        {"$project": {
            "_id": 0,
    ...
    rss_posts_pipeline = [
        ...
        {"$lookup": {
            ...
            "foreignField": "user_id",
            "pipeline": [
                {"$project": {"user_id": 1, "name": 1, "picture": 1, "is_verified": 1, "is_media": 1, "_id": 0}}
            ],
            "as": "user_data"
        }},
        {"$unwind": {"path": "$user_data", "preserveNullAndEmptyArrays": True}},
    ...
    if mixed_posts:
        last_post = mixed_posts[-1]
        next_cursor = f"{last_post.get('created_at', '')}:{last_post.get('post_id', '')}"
        # Add cursor info to response metadata (client should extract from last item)
...
async def get_fresh_posts(request: Request, limit: int = 20, seen_ids: str = ""):
    """Get fresh posts that user hasn't seen yet - mix of user posts and RSS
    ...
    Returns:
        Fresh posts mixed randomly between user posts and RSS
    """
    import random
    ...
    base_filter = {"moderation_status": {"$ne": "rejected"}}
    if seen_list:
    ...
    # Mix randomly
    all_posts = user_posts + rss_posts
    random.shuffle(all_posts)
    mixed = all_posts[:limit]
...
async def delete_user_post(post_id: str, request: Request):
    ...
    # Find the post
    post = await db.posts.find_one({"post_id": post_id}, {"_id": 0, "user_id": 1, "media_url": 1})
    ...
    logger.info(f"User {user.user_id} deleted post {post_id}")
...
async def get_user_posts(
    ...
    """Get user posts with privacy check and cursor pagination"""
    # Enforce limit bounds (max 50 for profile)
    ...
    projection = {
        "_id": 0, "post_id": 1, "user_id": 1, "content_type": 1, "media_url": 1,
        "caption": 1, "likes_count": 1, "comments_count": 1, "created_at": 1, "location": 1
    }
    
    posts = await db.posts.find(query, projection).sort([("created_at", -1), ("post_id", -1)]).limit(limit).to_list(limit)
    
    # Add cursor to each post
    for post in posts:
        post['_cursor'] = f"{post.get('created_at', '')}:{post.get('post_id', '')}"
    
    next_cursor = posts[-1]['_cursor'] if posts else None
...
async def send_friend_request(request: Request):
    ...
    try:
        ...
        if receiver and receiver.get("device_tokens"):
            for token in receiver["device_tokens"]:
                ...
                await push_service.send_to_device(
                    token,
                    "Demande d'ami",
                    f"{sender_name} souhaite être votre ami",
        ...
        await db.notifications.insert_one({
            ...
            "type": "friend_request",
            "title": "Demande d'ami",
            "message": f"{sender_name} souhaite être votre ami",
...
async def accept_friend_request(request_id: str, request: Request):
    ...
    try:
        ...
        if friend_request:
            ...
            accepter_name = accepter.get('name', 'Quelqu un')
            ...
            if sender and sender.get("device_tokens"):
                for token in sender["device_tokens"]:
                    await push_service.send_to_device(
                        token,
                        "Demande acceptée !",
                        f"{accepter_name} a accepté votre demande d'ami",
...
# NOTE: CORS middleware already configured at app startup (line ~166)
# DO NOT add duplicate CORS middleware here
...
def hash_admin_password(password: str) -> str:
    """Hash admin password using bcrypt (secure)"""
    return bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')

def verify_admin_password(password: str, stored_hash: str) -> tuple:
    """Verify admin password with SHA256 -> bcrypt migration support"""
    # Try bcrypt first (new format)
    if stored_hash.startswith('$2'):
        try:
            is_valid = bcrypt.checkpw(password.encode('utf-8'), stored_hash.encode('utf-8'))
            return is_valid, None  # No migration needed
        except:
            pass
    
    # Try SHA256 (old format) - migrate if valid
    sha256_hash = hashlib.sha256(password.encode()).hexdigest()
    if sha256_hash == stored_hash:
        # Password valid but using old hash - return new bcrypt hash for migration
...
async def admin_login(request: Request):
    ...
    # Verify password with bcrypt (supports SHA256 migration)
    is_valid, new_hash = verify_admin_password(password, admin["password_hash"])
    
    if email != admin["email"] or not is_valid:
        raise HTTPException(status_code=401, detail="Identifiants incorrects")
    
    # Migrate to bcrypt if using old SHA256 hash
    if new_hash:
        await db.admin_users.update_one(
            {"admin_id": admin["admin_id"]},
            {"$set": {"password_hash": new_hash, "hash_type": "bcrypt"}}
        )
        logger.info(f"Admin password migrated to bcrypt")
...
async def start_auto_publisher():
    """Start the RSS feed publisher background task - RSS only, no auto-generated posts"""
    import asyncio
    
    async def cleanup_auto_generated_posts():
        """Delete ONLY auto-generated posts (with random Unsplash images) - keep RSS articles"""
        try:
            # Delete auto-generated posts (NOT RSS articles)
            result = await db.posts.delete_many({
                "$or": [
                    {"is_auto_published": True},
                    {"post_id": {"$regex": "^auto_"}},
            ...
            if result.deleted_count > 0:
                logger.info(f"🗑️ Supprimé {result.deleted_count} posts auto-générés (images Unsplash)")
                await feed_cache.clear()
    ...
    async def initial_cleanup_and_rss():
        """Clean up auto-generated posts and fetch real RSS articles on startup"""
        try:
            await asyncio.sleep(10)  # Wait for DB to be ready
            
            # First, delete all auto-generated posts (fake posts with Unsplash images)
            deleted_auto = await cleanup_auto_generated_posts()
            
            # Clean up old RSS posts (older than 2 days)
            deleted_rss = await cleanup_old_rss_posts()
            
            # Count remaining posts
            total_posts = await db.posts.count_documents({})
            rss_posts = await db.posts.count_documents({"is_rss_article": True})
            user_posts = total_posts - rss_posts
            
            logger.info(f"📊 Posts: {user_posts} utilisateurs + {rss_posts} RSS = {total_posts} total")
    ...
    async def rss_publish_task():
        """Background task that fetches and publishes RSS articles 2x per day"""
        while True:
            try:
                # Wait 12 hours between refreshes
                await asyncio.sleep(12 * 3600)
                
                logger.info("🔄 Démarrage du rafraîchissement RSS...")
                
                # Clean up old RSS posts
    ...
    # Run initial cleanup and RSS fetch
...
async def run_ai_audit(request: Request):
    """Run a structured code audit"""
    user = await get_ai_user(request)
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")
    
    data = await request.json()
    audit_type = data.get("audit_type", "code_quality")
    code_snippet = data.get("code_snippet")
    file_path = data.get("file_path")
    session_id = data.get("session_id", f"audit_{audit_type}_{uuid.uuid4().hex[:8]}")
    
    result = await ai_agent.run_audit(
...
async def get_emergent_reports(request: Request, limit: int = 20):
    """Get all Emergent reports generated by the AI agent"""
    user = await get_ai_user(request)
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")
    
    reports = await ai_agent.get_emergent_reports(limit)
    return {"reports": reports}


@api_router.get("/ai/export")
async def export_audit_summary(request: Request):
    """Export a complete summary of all audits for Emergent"""
    user = await get_ai_user(request)
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")
    
    summary = await ai_agent.export_audit_summary()
    return summary
...
async def get_cloudinary_signature(
    ...
    user_folder = f"{folder}/{current_user.user_id}"
    
    signature_data = generate_upload_signature(user_folder, resource_type)
    
    if not signature_data:
        raise HTTPException(status_code=503, detail="Could not generate signature")
...
async def cloudinary_status():
    ...
    return {"enabled": is_cloudinary_enabled()}

@api_router.post("/cloudinary/upload")
async def cloudinary_upload(request: Request):
    """Backend upload for admin/special cases"""
    current_user = await get_current_user(request)
    if not current_user:
        raise HTTPException(status_code=401, detail="Authentication required")
    ...
    if content_type.startswith("video/"):
        result = await upload_video(content, f"{folder}/{current_user.user_id}")
    else:
        result = await upload_image(content, f"{folder}/{current_user.user_id}")
