...
import { postsApi, storiesApi, uploadApi, usersApi, cloudinaryApi } from '../lib/api';
...
const CreatePostPage = () => {
  ...
  const takePhoto = async () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      setCompressionMessage('Traitement de la photo...');
      
      try {
        const file = new File([blob], `photo_${Date.now()}.jpg`, { type: 'image/jpeg' });
        const folder = postType === 'story' ? 'stories' : 'posts';
        
        setUploadProgress(30);
        const uploadResult = await cloudinaryApi.upload(file, folder);
        
        setUploadProgress(100);
        setFormData(prev => ({ ...prev, media_url: uploadResult.url }));
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      const cloudinaryEnabled = await cloudinaryApi.isEnabled();
      
      if (cloudinaryEnabled) {
        // Use Cloudinary for permanent cloud storage
        setCompressionMessage('Upload vers le cloud...');
        uploadResult = await cloudinaryApi.upload(file, postType === 'story' ? 'stories' : 'posts');
        
        if (uploadResult.success) {
          setFormData({ ...formData, media_url: uploadResult.url });
          setCompressionMessage('Image uploadée sur le cloud !');
          toast.success('Image uploadée avec succès !');
        } else {
          throw new Error(uploadResult.error || 'Erreur Cloudinary');
        }
      } else {
      ...
      console.error('Upload error:', error);
      const errorMsg = error.response?.data?.detail || error.message || 'Erreur lors du téléchargement';
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Button
                ...
                onClick={startCamera}
                disabled={uploading}
                className="py-8 rounded-2xl bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white flex flex-col items-center gap-2"
              >
                <Camera size={28} />
