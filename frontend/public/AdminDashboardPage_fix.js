...
const AdminDashboardPage = () => {
  ...
  const tabs = [
    { id: 'ai-agent', label: 'Agent IA', icon: Bot, link: '/admin/ai-agent' },
  ...
  return (
    <div className="min-h-screen bg-[#1A1A2E] text-white">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-[#16213E] border-r border-white/10 flex flex-col">
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          {tabs.map((tab) => (
            tab.link ? (
              <Link
                key={tab.id}
