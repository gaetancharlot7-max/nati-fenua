...
async def create_critical_indexes_for_scale(db) -> dict:
    """Index critiques pour supporter 5000+ utilisateurs simultanés"""
    created = []
    try:
        # USER_SESSIONS - Critical for auth at scale
        await db.user_sessions.create_index([("session_token", 1)], unique=True, background=True)
        created.append("user_sessions.session_token")
        
        await db.user_sessions.create_index([("expires_at", 1)], expireAfterSeconds=0, background=True)
        created.append("user_sessions.expires_at_ttl")
        
        await db.user_sessions.create_index([("user_id", 1)], background=True)
        created.append("user_sessions.user_id")
        
        # LIKES - High traffic
        await db.likes.create_index([("post_id", 1), ("user_id", 1)], unique=True, background=True)
        created.append("likes.post_user")
        
        # REACTIONS - High traffic
        await db.reactions.create_index([("post_id", 1), ("user_id", 1)], background=True)
        created.append("reactions.post_user")
        
        # COMMENTS - Medium-high traffic
        await db.comments.create_index([("post_id", 1), ("created_at", -1)], background=True)
        created.append("comments.post_created")
        
        # FOLLOWS - Social graph
        await db.follows.create_index([("follower_id", 1), ("following_id", 1)], unique=True, background=True)
        created.append("follows.follower_following")
        
        await db.follows.create_index([("following_id", 1)], background=True)
        created.append("follows.following_id")
        
        # FRIEND_REQUESTS
        await db.friend_requests.create_index([("to_user_id", 1), ("status", 1)], background=True)
        created.append("friend_requests.to_user_status")
        
        logger.info(f"Created {len(created)} critical indexes for scale")
        
    except Exception as e:
        logger.error(f"Error creating scale indexes: {e}")
